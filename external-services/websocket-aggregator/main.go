...
type Config struct {
	...
	Compatibility struct {
		...
		WebSocketDeprecationWarning bool `yaml:"websocket_deprecation_warning"`
	} `yaml:"compatibility"`
	MongoDB struct {
		URI      string `yaml:"uri"`
...
func (wsa *WebSocketAggregator) publishAggregatedEvent(event *AggregatedEvent) {
	...
	// Publish to NATS for horizontal-broadcast-service consumption
	...
	log.Printf("[AGGREGATOR] ✅ Published to NATS successfully! Event will be broadcast by horizontal-broadcast-service")

	// BACKWARD COMPATIBILITY: Legacy WebSocket support (deprecated)
	if wsa.config.Compatibility.EnableWebSocket {
		if wsa.config.Compatibility.WebSocketDeprecationWarning {
			log.Printf("[AGGREGATOR] ⚠️ DEPRECATION WARNING: Direct WebSocket broadcasting is deprecated. Please use horizontal-broadcast-service")
		}
		...
		if clientCount > 0 {
			log.Printf("[AGGREGATOR] 🌐 [LEGACY] Broadcasting to %d WebSocket clients (deprecated path)", clientCount)
...
func (wsa *WebSocketAggregator) startWebSocketServer() {
	// BACKWARD COMPATIBILITY: Only start WebSocket server if explicitly enabled
	if !wsa.config.Compatibility.EnableWebSocket {
		log.Printf("[AGGREGATOR] 🔄 WebSocket server disabled (use horizontal-broadcast-service). Set compatibility.enable_websocket=true to enable legacy mode")
		return
	}

	if wsa.config.Compatibility.WebSocketDeprecationWarning {
		log.Printf("[AGGREGATOR] ⚠️ DEPRECATION WARNING: WebSocket Aggregator's direct WebSocket functionality is deprecated")
		log.Printf("[AGGREGATOR] ⚠️ Please migrate to horizontal-broadcast-service for WebSocket client management")
		log.Printf("[AGGREGATOR] ⚠️ This legacy mode will be removed in a future version")
	}
	...
	go func() {
		log.Printf("[AGGREGATOR] 🌐 [LEGACY] WebSocket server started on port %s%s (read buffer: %d, write buffer: %d)", 
		...
		log.Printf("[AGGREGATOR] ⚠️ [LEGACY] This is deprecated - use horizontal-broadcast-service instead")
